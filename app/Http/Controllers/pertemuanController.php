...
class pertemuanController extends Controller
{
    ...
    {
        $data = Pertemuan::where('uuid', $uuid)->first();
        $mapel = Mapel::where('instruktur_id', $data->mapel_id)->first();
    ...
    {
        $data = Pertemuan::with(['mapel', 'absensi.user', 'komentar.user'])->findOrFail($id);
        $modul = Modul::where('pertemuan_id', $data->id)->get();
        return view('instruktur.pertemuan.jadwalShow', compact('data', 'modul'));
    }

    public function filterAbsensi()
    {
        // Logika untuk menampilkan filter absensi
        return view('admin.pertemuan.filterAbsensi'); // Ganti dengan view yang sesuai
    }
}