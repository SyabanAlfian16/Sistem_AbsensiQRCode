...
class mapelController extends Controller
{
    ...
    {
        // Asumsikan Anda menggunakan auth untuk mendapatkan instruktur yang sedang login
        $instruktur = auth()->user()->instruktur;

        if (!$instruktur) {
            // Handle kasus di mana instruktur tidak ditemukan
            return redirect()->back()->with('error', 'Data instruktur tidak ditemukan');
        }

        $data = Mapel::where('instruktur_id', $instruktur->id)->get();
